...
class LoginFrame extends JFrame {
    ...
    private void onLogin(ActionEvent e) {
        ...
        if (user != null && user.passwordHash.equals(Crypto.hash(p))) {
            ...
            new MainFrame(user, this).setVisible(true);// Aqui :)
        } else {
...
class MainFrame extends JFrame {
    MainFrame(User user, LoginFrame loginFrame) {// Aqui :)
        super("Sistema – " + user.username);
        JTabbedPane tabs = new JTabbedPane();
        tabs.add("Galería", new GalleryPanel(user, loginFrame));
...
class GalleryPanel extends JPanel {
    ...
    GalleryPanel(User u, LoginFrame loginFrame) {// Aqui :)
        ...
        JButton salirBtn = new JButton("Salir");//El boton nuevo
        salirBtn.addActionListener(e -> {
         ((JFrame) SwingUtilities.getWindowAncestor(salirBtn)).dispose(); // Cierra la ventana actual
    ...
    private void refresh() {
        ...
        for (GalleryItem  gi : user.gallery) {
            JPanel  thumbPanel = makeThumb(gi);
            thumbs.add(thumbPanel);
        }
        revalidate();
        repaint();
    }
    ...
    private JPanel makeThumb(GalleryItem gi) {
        ...
        JLabel descLabel = new JLabel(gi.description);
